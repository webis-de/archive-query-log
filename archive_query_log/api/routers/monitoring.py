...
def statistics(
    ...
    return _statistics(config=config)


def _progress(config: ConfigDependency) -> list[Progress]:
    return [
...
def progress(
    ...
    return _progress(config=config)
...
def home(
    ...
    statistics_list: list[Statistics] = _statistics(config=config)
    progress_list: list[Progress] = _progress(config=config)
    etag = str(
        hash(
            (
