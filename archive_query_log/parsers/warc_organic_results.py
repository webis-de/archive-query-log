...
from archive_query_log.namespaces import (
    NAMESPACE_WARC_ORGANIC_RESULTS_PARSER,
    NAMESPACE_ORGANIC_RESULT,
)
...
class _OrganicResultData(BaseModel):
    id: UUID
    rank: int
    content: str
    url: AnyHttpUrl
...
class WarcOrganicResultsParser(BaseModel, ABC):
    ...
    def id(self) -> UUID:
        return uuid5(
            NAMESPACE_WARC_ORGANIC_RESULTS_PARSER, self.model_dump_json()
        )
    ...
    ) -> list[_OrganicResultData] | None: ...


class XpathWarcOrganicResultsParser(WarcOrganicResultsParser):
    xpath: str
    url_xpath: str
    ...
    def parse(
        ...
        for i, element in enumerate(elements):
            ...
            if len(urls) == 0:
                raise RuntimeError("The web search result block does not contain a URL.")
            url = urls[0].strip()
            ...
            organic_result_id = uuid5(
                NAMESPACE_ORGANIC_RESULT,
                ":".join(organic_result_id_components),
            )
            organic_results.append(
                _OrganicResultData(
                    id=organic_result_id,
                    rank=i,
                    content=content,
                    url=AnyHttpUrl(url),
                    title=title,
...
def parse_serp_warc_organic_results_action(
    serp: Serp,
    warc_store: WarcStore,
    index_organic_results: str,
) -> Iterator[dict]:
    ...
    if (
        serp.warc_organic_results_parser is not None
        and serp.warc_organic_results_parser.should_parse is not None
        and not serp.warc_organic_results_parser.should_parse
    ):
        return

    for parser in WARC_ORGANIC_RESULTS_PARSERS:
        if not parser.is_applicable(serp):
            continue
        warc_organic_results = parser.parse(serp, warc_store)
        if warc_organic_results is None:
        ...
        for organic_result in warc_organic_results:
            organic_result = OrganicResult(
                ...
                content=organic_result.content,
                url=organic_result.url,
                title=organic_result.title,
                text=organic_result.text,
                parser=InnerParser(
                    id=parser.id,
                    should_parse=False,
                    last_parsed=utc_now(),
...
def parse_serps_warc_organic_results(
    config: Config,
    size: int = 10,
    dry_run: bool = False,
) -> None:
    ...
    changed_serps_search: Search = (
        ...
        .filter(
            ...
            & ~Term(warc_organic_results_parser__should_parse=False)
        )
        .query(
    ...
    if num_changed_serps > 0:
        ...
        actions = chain.from_iterable(
            parse_serp_warc_organic_results_action(
                ...
                config.es.index_organic_results,
            )
...
WARC_ORGANIC_RESULTS_PARSERS: Sequence[WarcOrganicResultsParser] = (
