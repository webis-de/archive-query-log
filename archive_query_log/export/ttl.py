...
class TtlExporter(Exporter, Generic[_D]):
...
def get_exporter(document_type: type[_D]) -> Exporter[_D]:
    if document_type in {BaseDocument}:
        return TtlExporter(document_type)
    else:
        raise ValueError(f"No exporter defined for document type {document_type}")
