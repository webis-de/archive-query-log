...
class SearchFormIdentifier:
    """
    Class that takes in a CSV-file containing services. The file needs to have the following columns:
    - rank
    - service (name with TLD e.g. 'amazon.com')
    - TLD
    The process_services method will look for input fields, search forms and search divs on the corresponding website,
    indicate if the service has such a field as well as document any HMTL-snippet found during the search
    """
    def __init__(self, csv_file='./ranked_services.csv', outfile_num=0, start_row=0, end_row=None):
    ...
    def process_services(self):
        '''
        Use pd.DataFrame.progress_apply to run the method check_url on all services.
        Store the results in an attribute out_df and save it in a csv-File.
        '''
        tqdm.pandas()
        self.out_df = self.df.copy(deep=True)

        # Search for the HTML elements and split the results into separate columns
        self.out_df['tmp'] = self.df['service'].progress_apply(func=self.check_url)
        self.out_df[['input', 'search_form', 'search_div', 'input_snippets', 'form_snippets', 'div_snippets']] = \
    ...
    def check_url(self, url: str):
        """
        Method to take in a service URL and look for any relevant tags in the HTML.
        It will look for any input tag and div or form with an id that has the term 'search' in it
        ...
        url = 'https://' + url if not 'http' in url else url

        # If regular requesting fails, try to get a recent snapshot in the internet archive
        try:
            response = self.session.get(url, timeout=10) # request.urlopen(url, timeout=10)
        except:
            try:
        ...
        found_input, input_snippets = find_search_tag(soup=soup, tag='form')
    ...
    def get_internet_archive_html(self, url: str, year=2022, byte_digits=4):
        ...
        It will return the most recent snapshot from the specified year with a minimum size
        ...
        :param byte_digits:     Defines the minimum size with a leading 5.
                                Examples byte_digits=4 -> Bytes >= 5,000; byte_digits=5 -> Bytes >= 50,000
        :return:                The response from the Internet Archive Snapshot
        """
        # Get the snapshots with a minimum number of bytes from the specified year and extract the timestamp of the most recent
        search_url = f'https://web.archive.org/cdx/search/cdx?url={url}&fl=original,timestamp,length&from={year}&' \
                     f'filter=mimetype:text/html&filter=statuscode:200&filter=length:.*[5-9][0-9]%7B{byte_digits-1},%7D.*'
        try:
            ...
            raise RuntimeError('Failed to request an internet archive snapshot')


    def services_no_search(self):
        return self.out_df[(self.out_df['input'] == False) & (self.out_df['search_form'] == False) &
                           (self.out_df['search_div'] == False)]
...
def find_input_tag(soup: BeautifulSoup):
    ...
    :param      soup:           A bs4.BeautifulSoup instance with the HTML to conduct tag search on
...
def find_search_tag(soup: BeautifulSoup, tag='form'):
    """
    Function to look for HTML-Elements with the specified tag that contain the pattern "search" in some way
    :param      soup:           A bs4.BeautifulSoup instance with the HTML to conduct tag search on
