...
def combine_alexa_manual(manual_csv: str, alexa_csv: str):
    """
    Function to combine the CSV created by search_forms.py with a manual collection of search engines
    :param manual_csv:  Path to the CSV containing a manual collection of search engines
    :param alexa_csv:   Path to the CSV containing the automatically created list of services
    ...
    # Create new alexa df with unique service names and store the duplicates to be added later
    a_df_unique = a_df.drop_duplicates(subset="service")
    a_df_merged = a_df.merge(a_df_unique, on=["rank", "rank"], how="left", indicator=True)
    ...
    merged_df = pd.concat([a_df_unique.set_index("service"),
                           m_df.set_index("service")], axis=1, join="outer").reset_index()
    merged_df = pd.concat([merged_df, a_df_dup], ignore_index=True)
    cols_reordered = ["rank", "service", "tld", "Search Category", "input", "search_form", "search_div",
